...
import { discount, formatMinor, syntheticIsin } from "../lib/domain";
import { useFunder, type Funder } from "../lib/funder";
...
function ReceivableRow({
  ...
  const noFunder = !funder.accountId;

  async function fund() {
    if (!funder.accountId) return;

    setBusy(true);
    setError("");
    try {
      const res = await transfer(receivable.securityId, funder.accountId, "1");
  ...
  return (
    ...
    that is a bug worth seeing. */}
        ...
        <td className="right">
          {!conn ? (
            <span className="badge">Connect to verify</span>
          ) : !info ? (
...
export function FundView({ conn }: { conn: Connection | null }) {
  ...
  const firstPriced = first
    ? discount(first.faceValueMinor, first.termDays, first.annualRate)
    : null;
  ...
  return (
    <div className="grid">
      ...
      {conn && first && firstPriced && (
        <FunderWallet
          funder={funder}
          freelancerEvmAddress={conn.evmAddress}
          proceedsMinor={firstPriced.proceedsMinor}
