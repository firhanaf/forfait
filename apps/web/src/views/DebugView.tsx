...
import { formatMinor, type Invoice } from '../lib/domain';
  ...
  return (
    <div className="grid">
      <section className="card">
        ...
        <p className="hint">
          Held in this browser only. The ledger does not answer &ldquo;which securities did
          I create&rdquo;, so the app has to remember. Forgetting one here does not touch
          the contract.
        </p>
            ...
            <tbody>
              {receivables.map((r) => (
                <tr key={r.securityId}>
                  ...
                  <td className="right">{formatMinor(r.faceValueMinor, r.currency)}</td>
                  <td className="right">
                    <button className="ghost" onClick={() => forgetReceivable(r.securityId)}>
                      Forget
                    </button>
                  </td>
                </tr>
        ...
        {receivables.length > 0 && (
          <button className="ghost" style={{ marginTop: 12 }} onClick={clearReceivables}>
            Clear index
          </button>
        )}
      </section>
      ...
      <section className="card">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          ...
          <button className="ghost" disabled={disabled} onClick={() => run('pause', () => pause(target))}>
            Pause
          </button>

          <button className="ghost" disabled={disabled} onClick={() => run('unpause', () => unpause(target))}>
            Unpause
          </button>
        ...
        {!conn && <p className="hint" style={{ marginTop: 12 }}>Connect a wallet first.</p>}
      </section>
...
function RegisterExisting() {
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      {error && <div className="verdict bad" style={{ marginTop: 8 }}>{error}</div>}
    </div>
  );
}
